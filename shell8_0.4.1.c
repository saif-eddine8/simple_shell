...
int main(void) {
    ...
    while (1) {
        ...
        char *exit_command = strtok(line, " ");
        if (strcmp(exit_command, "exit") == 0) {
            char *exit_status_str = strtok(NULL, " "); // Get the exit status argument
            int exit_status = 0; // Default exit status if not provided

            if (exit_status_str != NULL) {
                exit_status = atoi(exit_status_str); // Convert the argument to an integer
            }

            write(STDERR_FILENO, exit_status_str, 5);
            free(line);
            char newline = '\n';
            write(STDOUT_FILENO, &newline, 1);
            _exit(exit_status); // Exit the program with the provided status
        }
